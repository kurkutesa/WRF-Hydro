...
[micha@MS-SL6 gis_hires]$ cd NetCDF
[micha@MS-SL6 gis_hires]$ ncdump -h geo_em.d03.nc | grep corner
...
[micha@MS-SL6 gis_hires]$ gdalinfo geo_em.d03.nc | grep HGT_M
 SUBDATASET_26_NAME=NETCDF:"NetCDF/geo_em.d03.nc":HGT_M
 SUBDATASET_26_DESC=[1x222x120] HGT_M (32-bit floating-point)
[micha@MS-SL6 gis_hires]$ gdal_translate -of AAIGrid -a_nodata -9999 NETCDF:"geo_em.d03.nc":HGT_M ../ascii/geo_em_hgt.asc
[micha@MS-SL6 gis_hires]$ cd ..
...
#---------------------------------------------------#
# Start GRASS in EPSG:4326 CRS and create new point
GRASS 7.0.svn (WGS84):~/geodata/HydroSHEDS >echo "33.27399,29.0575" | v.in.ascii in=- out=llcorner separator=, 
...
# Get the 3-sec conditioned grids
...
GRASS 7.0.svn (WGS84):~/geodata/HydroSHEDS > for d in n*; do \
    tile=`basename $d _con`; r.in.gdal input=$d/$d/w001001.adf output=$tile; 
done
GRASS 7.0.svn (WGS84):~/geodata/HydroSHEDS > g.region -p rast=`g.mlist rast pat=n* sep=,`
GRASS 7.0.svn (WGS84):~/geodata/HydroSHEDS > r.patch input=`g.mlist rast pat=n* sep=,` output=hydroshed_raw
# Remove individual tiles
GRASS 7.0.svn (WGS84):~/geodata/HydroSHEDS > g.mremove -f rast=n
...
# Use the eurasia raster to mask out DeadSea and Kinneret
r.mask eurasia_mask
...
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > g.proj -p
\-PROJ_INFO-------------------------------------------------
...
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > v.proj input=llcorner location=WGS84 mapset=Research
# Here are the new coordinates:
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > v.info -g llcorner
...
# Replace the xllcorner and yllcorner values with the projected points x-y coordinates 
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > head -6 ascii/geo_em_hgt.asc
ncols        120
nrows        222
...
NODATA_value  -9999
# Import the ASCII grid into GRASS
# Use the -e flag to set current region to the hgt raster, 
# and -o is necessary since the ASCII raster has no CRS definition
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > r.in.gdal -e -o ascii/geo_em_hgt.asc out=hgt

# Reproject the stations vector points layer
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > v.proj input=stations location=WGS84 mapset=Research output=stations

# Reproject the raster; first use the r.proj -g flag to set the necessary extents...
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > g.region `r.proj -g input=hydrosheds_dem location=WGS84 mapset=Research`
# ... force resolution to exactly 100 m...
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > g.region -p -a res=100                                                                
...
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > r.proj input=hydrosheds_dem location=WGS84 mapset=Research output=dem
...
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > r.colors hgt_m rule=grey
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > r.colors dem rule=srtm
# Verify region settings (resolution should still be 100m) 
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > g.region -p
...
r.stream.snap input=stations out=snapped_stations streams=str radius=10
...
r.out.gdal -c --o in=str out=ascii/channelgrid.asc format=AAIGrid type=Int16 nodata=-9999
...
# Convert the above to cellsize = 100
...
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > g.mapset location=WGS84 mapset=Research
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > v.proj --o input=snapped_stations location=WRF_LCC mapset=Research
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > v.to.db snapped_stations option=coor columns="longitude,latitude" units=degrees
# Output to a csv file
GRASS 7.0.svn (WRF_LCC):~/work/IHS/gis_hires > v.out.ascii -c --o input=snapped_stations \
    out=ascii/stations.csv columns="station_id,stat_num,stat_name" separator=,
